(() => {
  "use strict";
  const e = () => {
      const e = document.querySelector("#content");
      e.className = "flex flex-col";
      const t = document.createElement("div");
      t.className = "relative flex h-[50vh]";
      const n = document.createElement("div");
      n.className = "absolute top-[25vh] flex flex-wrap z-[10]";
      const l = document.createElement("h1");
      (l.className =
        "z-10 px-6 font-bold text-center text-white/90 customClamp"),
        (l.textContent =
          "Every dish is crafted with passion and the finest ingredients."),
        (l.textContent = l.textContent.toUpperCase());
      const a = document.createElement("img");
      (a.src = "./img/photo-1521917441209-e886f0404a7b.avif"),
        (a.className =
          "absolute top-0 z-0 object-cover object-center w-full h-full");
      const c = document.createElement("div");
      c.className = "flex flex-col gap-6 p-6";
      const o = document.createElement("div");
      o.className = "z-10 flex flex-col w-full h-full pb-12 max-w-[50vw]";
      const s = document.createElement("div");
      s.className =
        "flex flex-col items-end w-full h-full gap-2 max-w-[50vw] ml-auto";
      const d = document.createElement("p");
      (d.className = "z-10 text-white clampy"),
        (d.textContent =
          "Welcome to Daily Kitchen, where passion meets quality in every meal. We carefully select the finest ingredients and prepare each dish with love, so you can enjoy comforting flavors made fresh every day.");
      const i = document.createElement("p");
      (i.className = "z-10 text-right text-white clampy"),
        (i.textContent =
          "Order online from Daily Kitchen and enjoy your favorite meals wherever you are. Fresh ingredients, carefully prepared dishes, and comforting flavors just a few clicks away.");
      const r = document.createElement("button");
      (r.className = "btn btn-soft btn-wide"), (r.textContent = "Order online");
      const m = document.createElement("button");
      (m.className = "btn btn-soft btn-wide"),
        (m.textContent = "View our menu"),
        e.appendChild(t),
        e.appendChild(c),
        t.appendChild(n),
        t.appendChild(a),
        n.appendChild(l),
        c.appendChild(o),
        o.appendChild(d),
        s.appendChild(i),
        s.appendChild(m),
        o.appendChild(r),
        c.appendChild(s);
    },
    t = () => {
      const e = document.querySelector("#content");
      a();
      const t = document.createElement("div");
      t.className =
        "relative flex items-center w-full h-[130vh] shadow-lg bg-red-400";
      const n = document.createElement("h1");
      (n.className =
        "relative z-10 px-6 font-bold text-center text-white customClamp"),
        (n.textContent = "menu page."),
        (n.textContent = n.textContent.toUpperCase()),
        t.appendChild(n),
        e.appendChild(t);
    },
    n = () => {
      const e = document.querySelector("#content");
      a();
      const t = document.createElement("div");
      t.className =
        "relative flex items-center w-full h-[130vh] bg-gray-400 shadow-lg";
      const n = document.createElement("h1");
      (n.className =
        "relative z-10 px-6 font-bold text-center text-white customClamp"),
        (n.textContent = "about page."),
        (n.textContent = n.textContent.toUpperCase()),
        t.appendChild(n),
        e.appendChild(t);
    },
    l = () => {
      const a = document.querySelector("#content"),
        c = document.createElement("header");
      c.className =
        "absolute top-10 left-1/2 z-20 -translate-x-1/2 flex items-center justify-around w-full h-12 gap-1 px-4 max-w-[95vw] glass sm:max-w-[85vw] md:max-w-[70vw] lg:max-w-[55vw] xl:max-w-[45vw]";
      const o = document.createElement("h1");
      (o.textContent = "@Daily Kitchen"),
        (o.className = "text-lg font-bold text-white"),
        (o.textContent = o.textContent.toUpperCase());
      const s = document.createElement("nav");
      s.className = "flex items-center gap-6";
      const d = document.createElement("button");
      (d.className = "btn-custom"),
        (d.textContent = "Home"),
        d.addEventListener("click", function () {
          (a.innerHTML = ""), l(), e();
        });
      const i = document.createElement("button");
      (i.className = "btn-custom"),
        (i.textContent = "Menu"),
        i.addEventListener("click", function () {
          (a.innerHTML = ""), t();
        });
      const r = document.createElement("button");
      (r.className = "btn-custom"),
        (r.textContent = "About"),
        r.addEventListener("click", function () {
          (a.innerHTML = ""), n();
        }),
        s.appendChild(d),
        s.appendChild(i),
        s.appendChild(r),
        c.appendChild(o),
        c.appendChild(s),
        a.appendChild(c);
    },
    a = l;
  a(),
    e(),
    (() => {
      const e = document.querySelector("#content"),
        t = document.createElement("footer");
      t.className =
        "p-10 bg--500 footer sm:footer-horizontal bg-neutral text-neutral-content";
      const n = document.createElement("aside"),
        l = document.createElement("div");
      l.innerHTML =
        '<svg\n      width="50"\n      height="50"\n      viewBox="0 0 24 24"\n      xmlns="http://www.w3.org/2000/svg"\n      fill-rule="evenodd"\n      clip-rule="evenodd"\n      class="fill-current">\n      <path\n        d="M22.672 15.226l-2.432.811.841 2.515c.33 1.019-.209 2.127-1.23 2.456-1.15.325-2.148-.321-2.463-1.226l-.84-2.518-5.013 1.677.84 2.517c.391 1.203-.434 2.542-1.831 2.542-.88 0-1.601-.564-1.86-1.314l-.842-2.516-2.431.809c-1.135.328-2.145-.317-2.463-1.229-.329-1.018.211-2.127 1.231-2.456l2.432-.809-1.621-4.823-2.432.808c-1.355.384-2.558-.59-2.558-1.839 0-.817.509-1.582 1.327-1.846l2.433-.809-.842-2.515c-.33-1.02.211-2.129 1.232-2.458 1.02-.329 2.13.209 2.461 1.229l.842 2.515 5.011-1.677-.839-2.517c-.403-1.238.484-2.553 1.843-2.553.819 0 1.585.509 1.85 1.326l.841 2.517 2.431-.81c1.02-.33 2.131.211 2.461 1.229.332 1.018-.21 2.126-1.23 2.456l-2.433.809 1.622 4.823 2.433-.809c1.242-.401 2.557.484 2.557 1.838 0 .819-.51 1.583-1.328 1.847m-8.992-6.428l-5.01 1.675 1.619 4.828 5.011-1.674-1.62-4.829z"></path>\n    </svg>';
      const a = document.createElement("p");
      (a.innerHTML =
        "\n    Daily Kitchen.\n    <br />\n    Providing reliable food since 2025\n  "),
        n.appendChild(l),
        n.appendChild(a);
      const c = document.createElement("nav");
      (c.innerHTML =
        '<h6 class="footer-title">Social</h6>\n    <div class="grid grid-flow-col gap-4">\n      <a>\n        <svg\n          xmlns="http://www.w3.org/2000/svg"\n          width="24"\n          height="24"\n          viewBox="0 0 24 24"\n          class="fill-current cursor-pointer">\n          <path\n            d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"></path>\n        </svg>\n      </a>\n      <a>\n        <svg\n          xmlns="http://www.w3.org/2000/svg"\n          width="24"\n          height="24"\n          viewBox="0 0 24 24"\n          class="fill-current cursor-pointer">\n          <path\n            d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"></path>\n        </svg>\n      </a>\n      <a>\n        <svg\n          xmlns="http://www.w3.org/2000/svg"\n          width="24"\n          height="24"\n          viewBox="0 0 24 24"\n          class="fill-current cursor-pointer">\n          <path\n            d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"></path>\n        </svg>\n      </a>\n    </div>'),
        e.appendChild(t),
        t.appendChild(n),
        t.appendChild(c);
    })();
})();
